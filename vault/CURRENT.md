...
**11a.4 真机验收与收口**：自动化已全部通过（11a.1 策略层 commit `3e97619`；11a.2/11a.3 实现完成并经 2026-07-19 需求修改：双开右悬停直进完整面板、滚动指示固定信息左侧；前端 423 tests、Rust lib 142/1 全绿）。当前工作区改动未提交，等待：① 用户 Windows 真机矩阵（穿透/悬停组合、胶囊 240×80 命中区、悬停时序、经典/新样式切换、基线回归）；② 用户逐次授权 feat/docs 提交；③ 验收通过后稳定文件追加勿动清单并轮转下一动作。
