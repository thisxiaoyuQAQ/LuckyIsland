...
> 日期：2026-07-10
> 最终状态：✅ 已实施并于 2026-07-11 完成三类 Provider 真机验收；本文件保留为设计基线，不是当前执行清单
> 验收与当前事实：[`vault/08a-AI请求取消.md`](../../../vault/08a-AI请求取消.md)、[`vault/08-AI助手.md`](../../../vault/08-AI助手.md)、[`docs/开发进度.md`](../../开发进度.md)
> 偏离摘要：Codex 最终采用后端天气/DDG 预搜索注入证据，不依赖 `codex --search`；下文“待真机”与未提交约束均为 2026-07-10～11 的历史快照
...
### 16.2 历史待真机项（已于 2026-07-11 关闭）

以下条目记录 2026-07-10～11 验收过程中的中间状态，不再是当前待办：

- Chat API 的实时问答与 `providerUsed`/日志一致性已真机通过；
- Claude/Codex CLI 和 Chat API 两类取消，以及取消 A 后立即发送 B 的 late-result 隔离已真机通过；
- Provider 切换成功路径与三方一致性已真机通过，旧 BUG 不因本历史规格重开；
- 按需录音提示已于 2026-07-11 由用户确认修复，静音、模型/设备失败清理由自动化约束。

截至 2026-07-11，Provider 切换矩阵、三类 Provider 取消、A→B 竞态、Codex 联网与按需录音提示均已真机收敛。最终状态和证据以本文顶部链接的 Vault 与开发进度为准；当时的未提交要求也已被后续收尾指令替代。
