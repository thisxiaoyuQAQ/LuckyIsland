...
> 日期：2026-07-13
> 状态：已获用户逐节批准；详细计划与更新插件接入（`7485835`）已完成；2026-07-14 按确认顺序完成 FIX-10A-02 与 Phase 1 / Task 2 纯归约器，当前 Task 3 处于 `EffectPlan` RED
> 选定方案：增量功能 + 统一窗口策略层
> 实施范围：需求 1、2、3、5、6，以及 DOC-10A-03 新增的需求 7（副屏恢复后主动移回已保存副屏）
...
5. 鼠标悬停可自动展开灵动岛，并可配置；
6. 天气页展示未来七天天气；
7. 所选副屏断开并回退主屏后，副屏恢复时由应用主动移回已保存副屏。
...
- 所选副屏断开时继续主动回退主屏并保留用户选择。
- 已保存副屏恢复后，由 LuckyIsland 主动把岛移回该副屏；不得依赖 Windows 偶发的自动移回行为。
- 恢复动作保留当前偏移与用户期望状态，不应无条件抢焦点；自动化覆盖断开/恢复状态机，位置、DPI 与焦点行为另做 Windows 真机验收。
...
关于页 ──> Updater / Opener（Windows 安装由 NSIS 退出并重启）
...
- 多屏选择、运行时显示器断开回退或恢复主动移回后，下一轮按岛实际所在显示器重新判定。
- 应用退出时停止线程/任务。
...
- Windows 安装成功路径由 updater 启动 NSIS 并退出当前进程，不额外引入 process 插件或 JS `relaunch()`；
...
- 副屏断开时主动回退主屏；恢复已保存副屏时应用主动移回，保留偏移与期望状态且不无条件抢焦点；
...
- 副屏断开后回退主屏，恢复后由应用主动移回已保存副屏；偏移与用户期望状态保留，窗口不无故抢焦点；
...
- 2026-07-14 续接时工作区已有 FIX-10A-01、滚轮导航/启动时存储顺序及相关文档改动；这些改动必须保留，不得以恢复模块 11 为由覆盖、清理或误纳入无关提交。
- 恢复模块 11 前先完成 `vault/CURRENT.md` 指定的 FIX-10A-02，并同步模块 10 进度；随后显式把唯一入口切换到模块 11。模块 11 不重复已由 `7485835` 完成的更新插件接入，从详细计划 Phase 1 / Task 2 开始。
- 每个功能点独立 commit；暂存时显式列出本任务路径，不混入其他未提交工作。
- 不把其他功能分支或工作区内容合并进 `main`。
...
- 增加 updater 依赖、Tauri 配置和 capability；Windows 不增加 process 插件；
...
- 需求 1、2、3、5、6 及需求 7（副屏恢复后主动移回已保存副屏）按已确认行为完成；
- 窗口效果全部经策略层归约，状态测试通过；
- 穿透、悬停、全屏和通知多屏真机验收通过；
- 更新器状态机、签名配置和发布资产校验通过自动化门禁；
