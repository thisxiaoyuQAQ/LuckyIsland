...
> 日期：2026-07-11
> 最终状态：✅ 已实施并于 2026-07-11 完成自动化与 Windows 真机验收；本文件保留为设计基线，不是当前执行清单
> 验收与当前事实：[`vault/02-灵动岛外壳.md`](../../../vault/02-灵动岛外壳.md)、[`docs/开发进度.md`](../../开发进度.md)
> 偏离摘要：实施阶段补入运行时副屏断开监听与主动回退；副屏恢复时应用仍不主动定位，曾观察到的 Windows 自动恢复不构成产品保证
> 范围：模块 02「灵动岛外壳」P0 多屏选择与配置持久化 + 运行时副屏断开回退；不包含模块 07 动画/性能，也不包含 F1.4 副屏恢复后自动跳回
...
- 运行时副屏断开：立即把灵动岛临时移到主屏（不改持久化选择），设置页同步显示回退提示；副屏恢复时不自动跳回窗口，需重启或手动重选。

## 2. 非目标

- 不自动把窗口跳回恢复后的副屏：副屏运行时断开已临时回退主屏后，副屏重新接入时不自动迁移窗口，需重启或手动重选（F1.4 的「跟随主屏热插拔自动迁移」仍为 P1，本轮仅做断开回退）。
