...
- 透明窗 + 毛玻璃，跟随系统深浅色
- 多屏选择 + 位置持久化：副屏断开自动回退主屏，重连后由应用主动移回原副屏
- 可配置整窗鼠标穿透、悬停展开和岛所在显示器真正全屏时自动隐藏（Windows 10/11）
- 托盘菜单、开机自启、单实例
...
**其他页面**
- **日历**：月视图 + 农历 / 节气 / 节日
- **天气**：uapis.cn 当前天气/预警 + Open-Meteo 未来最多七日预报、地点候选消歧、同地点离线缓存与部分降级；按城市独立请求状态机，切换/删除城市的晚到响应不会污染界面
...
- 联网搜索、流式问答、请求可取消（取消/晚到响应由 reducer 状态机保证一致）、对话历史
- `Alt+Space` 随时唤起
...
**设置**
- 独立设置窗口：总体 / 外观 / 页面管理 / 通知 / 终端 / 天气 / 股票 / AI / 语音 / 快捷键 / 时间组件 / 时间外观 / 关于
- 关于页提供脱敏诊断、stable 更新检查、安全下载/安装和 GitHub Release 备用入口
- 文本/数字字段统一草稿 + debounce/blur 提交 + 保存错误回显
...
| 显示 / 隐藏灵动岛 | `Alt+X` |
| 唤起 AI 助手 | `Alt+Space` |
| 切换鼠标穿透 | 默认未绑定，可在设置中自定义 |
| 切换页面 | `Alt+1~9` / `Alt+←` / `Alt+->` / 滚轮 |
| 托盘左键 | 显示 / 隐藏 |

## 技术栈

- **Tauri 2**（Rust 后端 + WebView2 渲染，安装包 ~17MB，含 sherpa-onnx 动态库）
- **React 19** + TypeScript + Vite
- **Tailwind CSS v4** + shadcn 风格 UI + motion 12 动画
- **rusqlite** 本地存储（PRAGMA user_version 版本化迁移）、**reqwest** 网络层、**chrono / nongli** 农历
- **xterm.js + portable-pty** 终端、**sherpa-onnx + cpal** 语音
- **tracing + tracing-appender**：按天轮转日志，写入前统一脱敏（Bearer / 命名参数 / 高熵裸 token）
- 测试：Vitest（Node 纯逻辑 + happy-dom React 生命周期）+ Rust `#[cfg(test)]`

## 工程质量

- **统一验证门禁**：`pnpm verify` 串行执行 TypeScript、前端 Vitest、三入口 Vite build、Rust fmt、严格 Clippy (`-D warnings`)、Rust lib、cargo check
- **IPC 边界 unknown 校验**：手写轻量守卫（不引 zod），AI / 通知 / 天气 / 股票 12 个 invoke 与事件 payload 统一 `invoke<unknown> + assertIpc`，畸形响应无法渗入组件
- **Tauri 事件订阅统一**：`useTauriEvent` / `useAsyncSubscription` 共享 hook，卸载前/后 resolve 与 StrictMode 双挂载都有真实 React 回归证据
- **SQLite 版本化 migration**：`PRAGMA user_version` + 每版事务 + 失败回滚 + 拒绝降级；空库 / 旧库 / 幂等 / 失败回滚 7/7 通过
- **真机矩阵**：副屏断开/恢复、NSIS 安装后 lucky-notify、整窗手动穿透、Release 冷启动与资源基线、24h 长跑、语音全链路 6 项已通过

## 性能

- 语音关闭基线 ~61MB，CPU < 1%
- 语音开启闲置 ~110MB（唤醒时懒加载 ASR）
- 安装包 ~17MB（thin LTO + strip；sherpa-onnx 走 shared 动态链接，4 个 DLL 随包打包）
...
src/                      前端（React）
  ...
  pages/                  App 层 hook（registry / useIslandSettings / useIslandEvents / useIslandNavigation）
  settings/               设置面板各子面板（含 useDraftField 共享草稿 hook）
  ai-palette/             AI 命令面板（含 aiPaletteState 请求 reducer）
  lib/                    settings KV、动画、拖拽、IPC unknown 守卫、共享主题/事件订阅 hook
src-tauri/src/            Rust 后端
  data/                   天气 / 股票 / 日历 / 待办 / time_api（一言/历史）
  ai/                     AI 路由 / provider / 历史
  voice/                  唤醒 / ASR / TTS
  storage/                SQLite（PRAGMA user_version 版本化迁移）+ 配置导入导出白名单
  logging.rs              按天轮转 + 脱敏日志（tracing）
