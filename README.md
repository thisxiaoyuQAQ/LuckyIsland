...
- 透明窗 + 毛玻璃，跟随系统深浅色
- 多屏选择 + 位置持久化：副屏断开时应用主动回退主屏并保留原选择；恢复时不承诺自动跳回
- 托盘菜单、开机自启、单实例
...
- Provider 按各自实现联网：Codex 使用后端天气/DDG 证据注入，Chat API 使用后端 DDG 工具，Claude CLI 可用自身 WebSearch
- AI 回答一次性完整返回；请求期间显示 pending 状态，支持真正取消和对话历史
- `Alt+Space` 随时唤起
...
- 应用配置以 `app_data_dir()/data.db` 中的 SQLite `settings(key,value)` 为真源
- 两个动作热键可配置并检测冲突：显示/隐藏灵动岛、唤起 AI 助手
